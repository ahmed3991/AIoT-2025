...
#define TENSOR_ARENA_SIZE 50 * 1024  // 50 KB, مناسب لنموذج CNN صغير
...
void setup()
{
    ...
    // Load model
    model = tflite::GetModel(cnn_model_quant_tflite);    if (model->version() != TFLITE_SCHEMA_VERSION)
    {
...
void loop()
{
    ...
    if (buttonState == HIGH && takeNewPicture)
    {
        ...
        memcpy(interpreter->input(0)->data.int8, model_input_data, MODEL_INPUT_SIZE * sizeof(int8_t));
        ...
        lcd.setCursor(0, 1);
        lcd.print("Pred: " + String(class_names[predicted_class]));
